document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!tabla) {
    console.error("No se encontró la tabla con el ID 'tablaTareas'.");
    return;
  }
  ...
  columnas.forEach((columna, index) => {
    ...
    if (estadoIndex !== -1 && index === estadoIndex) {
      ...
      btnEstado.addEventListener("click", () => {
        // Lógica de cambio de estado para los 3 estados
        const estados = ["todos", "COMPLETADA", "PENDIENTE", "CANCELADA"];
        const colores = {
          "todos": "btn-warning text-dark",
          "COMPLETADA": "btn-success",
          "PENDIENTE": "btn-danger",
          "CANCELADA": "btn-secondary"
        };
        const textos = {
          "todos": "Todos",
        ...
        btnEstado.textContent = textos[nuevoEstado];
        btnEstado.className = `btn btn-sm me-2 mb-2 ${colores[nuevoEstado]}`;
  ...
  function aplicarFiltros() {
    ...
    filas.forEach(fila => {
      ...
      if (btnEstado && btnEstado.dataset.estado !== "todos") {
        const estadoFiltrar = btnEstado.dataset.estado.toLowerCase();
        if (estadoIndex !== -1) {
          const celdaEstado = fila.cells[estadoIndex];
          const estadoEnTabla = celdaEstado.textContent.trim().toLowerCase();

          if (estadoEnTabla !== estadoFiltrar) {
            visible = false;
          }
        }
      }
